...
FunC includes a low-level [standard library](/languages/tolk/vs-func/stdlib) in the `stdlib.fc` file, containing `asm` functions closely tied to [TVM instructions](/tvm/instructions).
...
- If `cs.load_uint(…)` was used to get a **copy**, a different approach is needed. [Read about mutability](/languages/tolk/vs-func/mutability).
