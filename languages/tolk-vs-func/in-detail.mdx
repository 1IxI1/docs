...
<Aside type="note" title="Compact version">
 [Tolk vs FunC: in short](/language/tolk-vs-func/in-short)
</Aside>
...
- The `!x` operator supports both `int` and `bool`.
- Conditions in `if` and similar statements accept both `int` (!= 0) and `bool`.
- Logical operators `&&` and `||` also accept both `bool` and `int`, preserving compatibility with expressions like `a && b`, where `a` and `b` are integers (!= 0).
- Arithmetic operators are restricted to integers. Only bitwise and logical operations are allowed for booleans:
