...
#### Syntax improvements
 - `;; comment` → `// comment`
 - `{- comment -}` → `/* comment */`
 - `#include` → `import`; strict rule: **import only what you use**
...
- See [the FunC-Tolk stdlib mapping](/languages/tolk-vs-func/stdlib).

### Language features
- No `~` tilde methods. See [mutability details](/languages/tolk-vs-func/mutability).
...
Code readability is improved, and gas usage is reduced.
